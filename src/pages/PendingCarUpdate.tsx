      ...
      try {
        ...
        if (firstBatchWithRemain) {
          ...
          const defaultVehicleType =
            firstBatchWithRemain.types.length > 0 ? firstBatchWithRemain.types[0] : 1
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Không thể tải danh sách xe chờ nhập liệu'
  ...
  const handleSubmit = async (values: CarFormValues) => {
    ...
    try {
      ...
      if (updatedBatch && updatedBatch.remain > 0) {
        ...
        const vehicleType =
          selectedVehicleType && updatedBatch.types.includes(selectedVehicleType)
            ? selectedVehicleType
                                ...
                                style={{
                                  border:
                                    selectedPublicCode === option.code
                                      ...
                                      : '1px solid #d9d9d9',
                                  backgroundColor:
                                    selectedPublicCode === option.code ? '#e6f7ff' : 'transparent',
                                  cursor: 'pointer',
                                  transition: 'all 0.2s',
                                }}
                                onClick={() => {
                                  ...
                                  const batch = allBatches.find(
                                    b => b.publicCode === option.code && b.remain > 0
                                  )
                                  if (batch) {
                      ...
                      <Row gutter={16} style={{ margin: 0 }}>
                        <Col span={12}>
                          <div>
                            ...
                            <Tag color="red">
                              {selectedBatch.remain}/{selectedBatch.quantity}
                            </Tag>
                          </div>
                        </Col>
                      </Row>
                      ...
                      <Row gutter={[16, 0]} style={{ margin: 0 }}>
                        <Col xs={24} md={12}>
                            ...
                            rules={[
                              {
                                validator: (_, value) => {
                                  ...
                                  // Tối đa 20 ký tự
                                  if (value.length > 20) {
                                    return Promise.reject(new Error('Dòng tối đa 20 ký tự'))
                                  }
                                  // Regex cho phép chữ cái tiếng Việt, chữ cái không dấu, chữ số
                                  return /^[a-zA-Z0-9ÀÁẢÃẠĂẰẮẲẴẶÂẦẤẨẪẬÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴĐđ\s]*$/.test(
                                    value
                                  )
                                    ? Promise.resolve()
                                    : Promise.reject(
                                        new Error(
                                          'Dòng chỉ được chứa chữ cái (có dấu/không dấu) và chữ số'
                                        )
                                      )
                                },
                              },
                          ...
                          <Form.Item label="Trọng lượng" name="weight">
                            <Input placeholder="Ví dụ: 110kg" size="large" />
                          </Form.Item>

                          <Form.Item label="Dung tích" name="cc">
                            <Input placeholder="Ví dụ: 125cc" size="large" />
                          </Form.Item>
