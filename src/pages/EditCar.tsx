...
const EditCar = () => {
  ...
  const handleSubmit = async (values: CarFormValues) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Không thể cập nhật thông tin xe'
      message.error(errorMessage)
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <div style={{ padding: 24 }}>
      <Space style={{ marginBottom: 24, width: '100%', justifyContent: 'space-between' }} wrap>
        ...
        <Space>
          <Button onClick={() => navigate(`/cars/${id}`)}>Hủy</Button>
                ...
                <Row gutter={16} style={{ margin: 0 }}>
                  ...
                  <Col span={12}>
                    <div>
                      ...
                      <Text>
                        {product.vehicleType === 1
                          ? 'Xe tay ga'
                          : product.vehicleType === 2
                            ? 'Xe số'
                            : product.vehicleType === 3
                              ? 'Xe côn tay'
